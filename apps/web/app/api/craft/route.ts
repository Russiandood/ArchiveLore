    ...
    try {
        ...
        if (Object.keys(inputsMat).length) {
            ...
            for (const field of Object.keys(inputsMat)) {
                ...
                if (have < need) return NextResponse.json({ ok: false, error: `need ${need} of ${field}` }, { status: 400 });
            }
        }
        if (Object.keys(inputsEss).length) {
            ...
            for (const field of Object.keys(inputsEss)) {
                ...
                if (have < need) return NextResponse.json({ ok: false, error: `need ${need} essence of ${field}` }, { status: 400 });
            }
        }
        if (sparkTier) {
